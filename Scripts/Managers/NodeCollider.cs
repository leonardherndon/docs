...
public class NodeCollider : MonoBehaviour, ICollidible
{
   ...
   public void HandleCollision(GameObject other, List<DamageDataBlock> damageBlockList = null, List<StatusEffectDataBlock> sEA = null)
   {
       ...
       switch (otherCoC.CSM.LumenMode)
       {
           case LumenMode.Light:
               AddMatchingCores(otherCoC);
               break;
           case LumenMode.Dark:
               RemoveMatchingCores(otherCoC);
               break;
       }
   ...
   public void AddMatchingCores(ICollidible other)
   {
       //Debug.Log(gameObject.name + "Add Matching Cores");
       FusionCore[] cores = GameManager.Instance.EMPTYFC;
       switch (CSM.LumenType)
       {
           case LumenType.Simple:
               cores = new[] { new FusionCore(GameColor.Red, true, false,0), new FusionCore(GameColor.Green,true,false,0 ), new FusionCore(GameColor.Blue, true, false,0)};
               break;
           case LumenType.Complex:
       ...
       foreach (var core in cores)
       {
           ...
           for (int i = 0; i < CSM.CoreInventory.Length; i++)
           {
               if (CSM.CoreInventory[i].ColorIndex == core.ColorIndex)
               {
                   if (core.IsActive)
                   {
                       CSM.CoreInventory[i].IsActive = true;
   ...
   public void RemoveMatchingCores(ICollidible other)
    {
        //Debug.Log(gameObject.name + "Remove Matching Cores");
