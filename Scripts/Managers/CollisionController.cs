...
public class CollisionController : SerializedMonoBehaviour, ICollidible
{

    [Header("Scriptable")] [SerializeField]
    private CollisionControllerData CCD;
    [Header("COLLISION")] 
    ...
    [SerializeField] private bool _doesNotReceiveCollisionDamage = false;

    public bool DoesNotReceiveCollisionDamage
    {
        get => _doesNotReceiveCollisionDamage;
        set => _doesNotReceiveCollisionDamage = value;
    }
    ...
    [OdinSerialize] private ILifeSystem _lifeSystem;
    ...
    public bool StatusEffectImmunityAll
    {
    ...
    public event OnEventGainedCore OnGainedCore = delegate { };
    public UnityEvent CoreChanged;
    ...
    public void InitCollisionController()
    {
        if (!CCD)
        {
            Debug.LogError("No Collision Controller Data Attached to " + gameObject.name + ". Please attach data to enable functionality");
            return;
        }
        ...
        _doesNotReceiveCollisionDamage = CCD.doesNotReceiveCollisionDamage;
        _doesVelocityDamage = CCD.doesVelocityDamage;
        
        _collisionColorMatchType = CCD.collisionColorMatchType;
        
        _threatGroup = CCD.threatGroup;
        _friendlyType = CCD.friendlyType;
        
        _damageGroup = CCD.damageGroup;
        _impactType = CCD.impactType;
        _damageImmunityType = CCD.damageImmunityType;
        damageDataBlocks = CCD.damageDataBlocks;
        
        _statusEffectImmunityAll = CCD.statusEffectImmunityAll;
        _statusEffectImmunityType = CCD.statusEffectImmunityType;
        statusEffectBlocks = CCD.statusEffectBlocks;
        
        if (damageDataBlocks.Count == 0)
            Debug.LogError("No DamageBlock Attached. Please attach a damage block to the CCD to initialize");
        if (statusEffectBlocks.Count == 0)
            Debug.LogError("No StatusEffectBlock Attached. Please attach a status effect block to the CCD to initialize");
   ...
   public void OnCollisionEnter(Collision other)
    {
        //Debug.Log("COLLISION DETECTED: " + gameObject.name + " [" + collisionObjectId +"] | " + other.gameObject.name);
        
        var collideObject = other.gameObject.GetComponent<ICollidible>();
        if (collideObject == null)
        {
            var collideParent = other.gameObject.GetComponentInParent<ICollidible>();
        ...
        //Debug.Log("Other Collision ID: " + collideObject.CollisionObjectId);
        CalcCollision(collideObject, other);
    }
   
   public void OnTriggerEnter(Collider other)
   {
       Debug.Log("TRIGGER DETECTED: " + gameObject.name + " [" + collisionObjectId +"] -----> " + other.gameObject.name);

       string otherLayerName = LayerMask.LayerToName(other.gameObject.layer);
       Debug.Log("[[" + otherLayerName +"]]");
       if (otherLayerName.Contains("Hurtbox"))
       {
           Debug.Log("This is the damage dealer ["+gameObject.name+"]");
           return;
       } 
   ...
   public void CalcCollision(ICollidible colliderObject, Collision otherCollision = null)
   {
       ...
       //Debug.Log("CALC COLLISION");
       var dA = colliderObject.DamageDataBlocks;
       var sEA = colliderObject.StatusEffectDataBlocks;
       
       if (colliderObject.IsLightNode) return;
       
       if (_doesNotReceiveCollisionDamage)
       {
           Debug.LogWarning(gameObject.name + " - This GameObject does not receive damage.");
           return;
       }
       ...
       if (colliderObject.DoesVelocityDamage)
       {
           ...
           GotVelocityDamaged?.Invoke(otherCollision, colliderObject.ImpactType, dA, colliderObject, velMultiplier);
           return;
       }
   ...
   public void HandleCollision(GameObject other, List<DamageDataBlock> damageBlockList, List<StatusEffectDataBlock> sEA = null)
   {
       ...
       if (passedCollisionTest)
       {
           ...
           switch (otherDamageGroup)
           {
               ...
               case DamageGroup.Key:
                   //GotDamaged?.Invoke(dA, DamageSeverity.Dampened);  DOES NO DAMAGE
       ...
       else //FAILING COLOR CHECK OR OBJECT IGNORES COLOR MATCHING
       {
           Debug.Log("Failed Collision Color Test: " + gameObject.name + " " + CSM.CurrentColor + " | " + otherCoC.gameObject.name + " " +otherCoC.CSM.CurrentColor);
   ...
   private bool CollisionColorTest(GameObject other)
   {
    ...
    {
        //Debug.Log("running Remove Matching Cores");
        foreach (var core in other.CSM.CoreInventory)
        {
            ...
            foreach (var thisCore in CSM.CoreInventory)
            {
                if (core.ColorIndex == thisCore.ColorIndex)
                {
                    // CSM.CoreInventory[i].IsActive = false;
                    CSM.DeactivateFusionCoreByColorIndex(core.ColorIndex);
                    isUnmatchedCore = false;
                    // StartCoroutine(CSM.CoreInventory[i].Regeneration());
                    break;
                }
            }
